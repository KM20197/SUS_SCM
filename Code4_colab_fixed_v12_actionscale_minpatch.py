...
class DoubleQLearningAgent:
    ...
    def init_bins(self, demand: np.ndarray, forecast: np.ndarray, lead_time_scale: float = 1.0) -> None:
    # 10 bins each -> grid 10x10 -> 100 states (default)
    # Scale inventory bins to reflect pipeline magnitudes (lead times), so states do not saturate.
    lt = float(max(1.0, lead_time_scale))
    inv_ref = demand * max(3.0, lt)
    self.state_bins = {
        "inventory": np.percentile(inv_ref, np.linspace(0, 100, 11)),
        "forecast": np.percentile(forecast, np.linspace(0, 100, 11)),
    }
    # Base order_scale on daily demand magnitude; pipeline scaling is applied when converting actions to quantities.
...
def lead_time_max(scenario: str) -> int:
"""Deterministic upper bound of lead time by scenario.

Used only to scale the AI policy's action-to-order mapping and inventory state bins
so the RL agent can express order quantities on the same magnitude as the pipeline dynamics.
This does not change baselines or other experiment settings.
"""
scenario = scenario.upper()
if scenario == "SUPPLY_DISRUPTION":
    return 71
...
def simulate_policy_episode(
    ...
    def compute_order_qty(t: int, d_t: float, uc: float, hist: np.ndarray) -> float:
        ...
        if policy == "AI":
            ...
            return float(a * agent.order_scale * float(lead_time_max(scenario)))
...
def train_agent(
    ...
    agent.init_bins(demand=d, forecast=fc, lead_time_scale=lead_time_max(scenario))
    ...
    for ep in range(episodes):
        ...
        for t in range(max_t - 1):
            ...
            order_qty = float(a * agent.order_scale * float(lead_time_max(scenario)))
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    manifest = {
        ...
        "ai_action_scale": "action_qty = action_index * order_scale * lead_time_max(scenario) (order_scale based on p90 daily demand)",
        "data_info": data_info,
    }
